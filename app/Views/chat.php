...
<body>
    ...
    <script>
        // const socket = new WebSocket("wss://192.168.1.34:8080/");
        const socket = new WebSocket("wss://templates.walisongosragen.com:8080/");

        socket.onmessage = function(event) {
            try {
                const data = JSON.parse(event.data);
                if (data.sender && data.message) {
                    document.getElementById("chatBox").innerHTML += `<p>${data.sender}: ${data.message}</p>`;
                } else if (data.type === 'chat_data_updated' && data.payload) {
                    document.getElementById("chatBox").innerHTML += `<p>${data.payload.sender}: ${data.payload.message}</p>`;
